...
{
	_1=`echo ${1} | tr - _`
	[ -z "${_1}" ] || eval mkdir -pv \${${_1}_src_base} || return 1
	case ${1} in
	'')
	...
	autogen)
		check_archive ${autogen_org_src_dir} ||
			wget -O ${autogen_org_src_dir}.tar.xz \
				http://ftp.gnu.org/gnu/autogen/rel${autogen_ver}/${autogen_name}.tar.xz || return 1;;
	xz)
...
{
	[ ${prefix}/src = `dirname ${0}` ] || cp -fv ${0} ${prefix}/src || return 1
...
install_native_tar()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${tar_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_cpio()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${cpio_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_xz()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${xz_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_bzip2()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${bzip2_org_src_dir} -j ${jobs} -k check || return 1
	...
	[ "${enable_check}" != yes ] ||
		make -C ${bzip2_org_src_dir} -j ${jobs} -k check || return 1
	cp -fv ${bzip2_org_src_dir}/libbz2.so.${bzip2_ver} ${prefix}/lib || return 1
	chmod a+r ${prefix}/lib/libbz2.so.${bzip2_ver} || return 1
	ln -fsv ./libbz2.so.${bzip2_ver} ${prefix}/lib/libbz2.so.`echo ${bzip2_ver} | cut -d . -f -2` || return 1
	cp -fv ${bzip2_org_src_dir}/bzlib.h ${prefix}/include || return 1
	cp -fv ${bzip2_org_src_dir}/bzlib_private.h ${prefix}/include || return 1
	update_search_path || return 1
}
...
install_native_gzip()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${gzip_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_wget()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${wget_org_src_dir} -j ${jobs} -k check || return 1
...
#	search_header glib.h glib-2.0 > /dev/null || install_native_glib || return 1
	...
	[ "${enable_check}" != yes ] ||
		make -C ${pkg_config_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_texinfo()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${texinfo_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_coreutils()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${coreutils_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_flex()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${flex_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_m4()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${m4_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_autoconf()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${autoconf_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_automake()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${automake_org_src_dir} -j ${jobs} -k check || return 1
	make -C ${automake_org_src_dir} -j ${jobs} install || return 1
}

install_native_autogen()
{
	[ -x ${prefix}/bin/autogen -a "${force_install}" != yes ] && return 0
#	which autoconf > /dev/null || install_native_autoconf || return 1
	fetch autogen || return 1
	unpack ${autogen_org_src_dir} ${autogen_src_base} || return 1
	[ -f ${autogen_org_src_dir}/Makefile ] ||
		(cd ${autogen_org_src_dir}
		./configure --prefix=${prefix} --build=${build} --disable-silent-rules) || return 1
	make -C ${autogen_org_src_dir} -j ${jobs} || return 1
	[ "${enable_check}" != yes ] ||
		make -C ${autogen_org_src_dir} -j ${jobs} -k check || return 1
	make -C ${autogen_org_src_dir} -j ${jobs} install || return 1
}
...
install_native_libtool()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${libtool_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_sed()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${sed_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_gawk()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${gawk_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_make()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${make_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_binutils()
{
	...
	[ -d ${binutils_src_dir_ntv} ] ||
		(unpack ${binutils_org_src_dir} ${binutils_src_base} &&
			mv -v ${binutils_org_src_dir} ${binutils_src_dir_ntv}) || return 1
...
#			CXXFLAGS="${CXXFLAGS} -Wno-error=unused-function"
	...
	[ "${enable_check}" != yes ] ||
		make -C ${binutils_src_dir_ntv} -j 1 -k check || return 1
...
install_native_kernel_header()
{
	...
	[ -d ${linux_src_dir_ntv} ] ||
		(unpack ${linux_org_src_dir} ${linux_src_base} &&
			mv -v ${linux_org_src_dir} ${linux_src_dir_ntv}) || return 1
...
install_native_gperf()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${gperf_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_glibc()
{
	...
	[ -d ${glibc_src_dir_ntv} ] ||
		(unpack ${glibc_org_src_dir} ${glibc_src_base} &&
			mv -v ${glibc_org_src_dir} ${glibc_src_dir_ntv}) || return 1
	...
	[ "${enable_check}" != yes ] ||
		make -C ${glibc_bld_dir_ntv} -j ${jobs} -k check || return 1
...
install_native_mpfr()
{
	...
	[ -d ${mpfr_src_dir_ntv} ] ||
		(unpack ${mpfr_org_src_dir} ${mpfr_src_base} &&
			mv -v ${mpfr_org_src_dir} ${mpfr_src_dir_ntv}) || return 1
	...
	[ "${enable_check}" != yes ] ||
		make -C ${mpfr_src_dir_ntv} -j ${jobs} -k check || return 1
...
install_native_mpc()
{
	...
	[ -d ${mpc_src_dir_ntv} ] ||
		(unpack ${mpc_org_src_dir} ${mpc_src_base} &&
			mv -v ${mpc_org_src_dir} ${mpc_src_dir_ntv}) || return 1
	...
	[ "${enable_check}" != yes ] ||
		make -C ${mpc_src_dir_ntv} -j ${jobs} -k check || return 1
...
install_native_readline()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${readline_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_gdb()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${gdb_bld_dir_ntv} -j ${jobs} -k check || return 1
...
install_native_zlib()
{
	...
	[ -d ${zlib_src_dir_ntv} ] ||
		(unpack ${zlib_org_src_dir} ${zlib_src_base} &&
			mv -v ${zlib_org_src_dir} ${zlib_src_dir_ntv}) || return 1
	(cd ${zlib_src_dir_ntv}
	./configure --prefix=${prefix}) || return 1
	make -C ${zlib_src_dir_ntv} -j ${jobs} || return 1
	[ "${enable_check}" != yes ] ||
		make -C ${zlib_src_dir_ntv} -j ${jobs} -k check || return 1
...
install_native_libpng()
{
	...
	[ -d ${libpng_src_dir_ntv} ] ||
		(unpack ${libpng_org_src_dir} ${libpng_src_base} &&
			mv -v ${libpng_org_src_dir} ${libpng_src_dir_ntv}) || return 1
	[ -f ${libpng_src_dir_ntv}/Makefile ] ||
		(cd ${libpng_src_dir_ntv}
		./configure --prefix=${prefix} --build=${build}) || return 1
	make -C ${libpng_src_dir_ntv} -j ${jobs} || return 1
	[ "${enable_check}" != yes ] ||
		make -C ${libpng_src_dir_ntv} -j ${jobs} -k check || return 1
...
install_native_libtiff()
{
	...
	[ -d ${tiff_src_dir_ntv} ] ||
		(unpack ${tiff_org_src_dir} ${tiff_src_base} &&
			mv -v ${tiff_org_src_dir} ${tiff_src_dir_ntv}) || return 1
	[ -f ${tiff_src_dir_ntv}/Makefile ] ||
		(cd ${tiff_src_dir_ntv}
		./configure --prefix=${prefix} --build=${build}) || return 1
	make -C ${tiff_src_dir_ntv} -j ${jobs} || return 1
	[ "${enable_check}" != yes ] ||
		make -C ${tiff_src_dir_ntv} -j ${jobs} -k check || return 1
...
install_native_libjpeg()
{
	...
	[ -d ${jpeg_src_dir_ntv} ] ||
		(unpack ${jpeg_org_src_dir} ${jpeg_src_base} &&
			mv -v ${jpeg_org_src_dir} ${jpeg_src_dir_ntv}) || return 1
	[ -f ${jpeg_src_dir_ntv}/Makefile ] ||
		(cd ${jpeg_src_dir_ntv}
		./configure --prefix=${prefix} --build=${build} --disable-silent-rules) || return 1
	make -C ${jpeg_src_dir_ntv} -j ${jobs} || return 1
	[ "${enable_check}" != yes ] ||
		make -C ${jpeg_src_dir_ntv} -j ${jobs} -k check || return 1
...
install_native_giflib()
{
	...
	[ -d ${giflib_src_dir_ntv} ] ||
		(unpack ${giflib_org_src_dir} ${giflib_src_base} &&
			mv -v ${giflib_org_src_dir} ${giflib_src_dir_ntv}) || return 1
	[ -f ${giflib_src_dir_ntv}/Makefile ] ||
		(cd ${giflib_src_dir_ntv}
		./configure --prefix=${prefix} --build=${build} --disable-silent-rules) || return 1
	make -C ${giflib_src_dir_ntv} -j ${jobs} || return 1
	[ "${enable_check}" != yes ] ||
		make -C ${giflib_src_dir_ntv} -j ${jobs} -k check || return 1
...
install_native_libXpm()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${libXpm_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_libwebp()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${libwebp_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_libffi()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${libffi_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_emacs()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${emacs_org_src_dir} -j ${jobs} -k check || return 1
...
#		+xterm_save \
	...
	[ -d ${vimdoc_ja_org_src_dir} ] ||
		...
		mv -fv ${vimdoc_ja_src_base}/vimdoc-ja-master ${vimdoc_ja_org_src_dir}) || return 1
	mkdir -pv ${prefix}/share/vim/vimfiles || return 1
...
install_native_grep()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${grep_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_global()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${global_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_pcre2()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${pcre2_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_the_silver_searcher()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${the_silver_searcher_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_graphviz()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${graphviz_org_src_dir} -j ${jobs} -k check || return 1
...
#	make -C ${doxygen_bld_dir_ntv} -j ${jobs} docs || return 1
	[ "${enable_check}" != yes ] ||
		make -C ${doxygen_bld_dir_ntv} -j ${jobs} -k tests || return 1
...
install_native_diffutils()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${diffutils_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_patch()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${patch_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_findutils()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${findutils_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_screen()
{
	...
	mkdir -pv ${prefix}/share/screen/utf8encodings || return 1
...
install_native_libevent()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${libevent_org_src_dir}-stable -j ${jobs} -k check || return 1
...
install_native_tmux()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${tmux_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_expect()
{
	...
	[ -f ${expect_org_src_dir}/Makefile ] ||
		...
		./configure --prefix=${prefix} --build=${build} --enable-threads \
			--enable-64bit --with-tcl=`search_library_dir tclConfig.sh`) || return 1
	make -C ${expect_org_src_dir} -j ${jobs} || return 1
	[ "${enable_check}" != yes ] ||
		make -C ${expect_org_src_dir} -j ${jobs} -k check || return 1
	make -C ${expect_org_src_dir} -j ${jobs} install || return 1
	ln -fsv ./${expect_name}/lib${expect_name}.so ${prefix}/lib || return 1
}

install_native_dejagnu()
{
#	[ -x ${prefix}/bin/dejagnu -a "${force_install}" != yes ] && return 0
	which expect > /dev/null || install_native_expect || return 1
	fetch dejagnu || return 1
	unpack ${dejagnu_org_src_dir} ${dejagnu_src_base} || return 1
	[ -f ${dejagnu_org_src_dir}/Makefile ] ||
		(cd ${dejagnu_org_src_dir}
		./configure --prefix=${prefix} --build=${build}) || return 1
	make -C ${dejagnu_org_src_dir} -j ${jobs} || return 1
	[ "${enable_check}" != yes ] ||
		make -C ${dejagnu_org_src_dir} -j ${jobs} -k check || return 1
	make -C ${dejagnu_org_src_dir} -j ${jobs} install || return 1
}
...
install_native_zsh()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${zsh_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_bash()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${bash_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_inetutils()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${inetutils_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_openssl()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${openssl_org_src_dir} -j ${jobs} -k test || return 1
...
install_native_openssh()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${openssh_org_src_dir} -j ${jobs} -k tests || return 1
...
install_native_asciidoc()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${asciidoc_org_src_dir} -j ${jobs} -k test || return 1
...
install_native_libxml2()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${libxml2_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_libxslt()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${libxslt_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_xmlto()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${xmlto_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_gettext()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${gettext_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_git()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${git_org_src_dir} -j ${jobs} V=1 -k test || return 1
...
install_native_mercurial()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${mercurial_org_src_dir} -j ${jobs} PYTHON=python -k check || return 1
...
install_native_sqlite()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${sqlite_autoconf_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_apr()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${apr_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_apr_util()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${apr_util_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_subversion()
{
	...
	[ -f ${subversion_org_src_dir}/Makefile ] ||
		...
		./configure --prefix=${prefix} --build=${build} --with-zlib=`get_prefix zlib.h` \
			...
			${strip:+--enable-optimize}) || return 1
	make -C ${subversion_org_src_dir} -j ${jobs} || return 1
	[ "${enable_check}" != yes ] ||
		make -C ${subversion_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_cmake()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${cmake_org_src_dir} -j ${jobs} -k test || return 1
...
install_native_libedit()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${libedit_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_swig()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${swig_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_llvm()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${llvm_bld_dir} -j ${jobs} -k check || return 1
...
install_native_libcxx()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${libcxx_bld_dir} -j ${jobs} -k check-libcxx || return 1
...
install_native_cfe()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${cfe_bld_dir} -j ${jobs} -k check-all || return 1
...
install_native_lld()
{
	...
	[ -d ${llvm_org_src_dir}/tools/lld ] ||
		...
		mv -v ${lld_org_src_dir} ${llvm_org_src_dir}/tools/lld) || return 1
...
install_native_lldb()
{
	...
	[ -d ${llvm_org_src_dir}/tools/lldb ] ||
		...
		mv -v ${lldb_org_src_dir} ${llvm_org_src_dir}/tools/lldb) || return 1
...
install_cross_binutils()
{
	...
	[ -d ${binutils_src_dir_crs} ] ||
		(unpack ${binutils_org_src_dir} ${binutils_src_base} &&
			mv -v ${binutils_org_src_dir} ${binutils_src_dir_crs}) || return 1
	...
	[ "${enable_check}" != yes ] ||
		make -C ${binutils_src_dir_crs} -j 1 -k check || return 1
...
install_cross_gcc_without_headers()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${gcc_bld_dir_crs_1st} -j ${jobs} -k check-gcc || return 1
...
install_cross_kernel_header()
{
	...
	[ -d ${linux_src_dir_crs} ] ||
		(unpack ${linux_org_src_dir} ${linux_src_base} &&
			mv -v ${linux_org_src_dir} ${linux_src_dir_crs}) || return 1
...
install_cross_glibc_headers()
{
	...
	[ -d ${glibc_src_dir_crs_hdr} ] ||
		(unpack ${glibc_org_src_dir} ${glibc_src_base} &&
			mv -v ${glibc_org_src_dir} ${glibc_src_dir_crs_hdr}) || return 1
...
install_cross_gcc_with_glibc_headers()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${gcc_bld_dir_crs_2nd} -j ${jobs} -k check-gcc || return 1
	...
	[ "${enable_check}" != yes ] ||
		make -C ${gcc_bld_dir_crs_2nd} -j ${jobs} -k check-target-libgcc || return 1
...
install_cross_1st_glibc()
{
	[ -d ${glibc_src_dir_crs_1st} ] ||
		(unpack ${glibc_org_src_dir} ${glibc_src_base} &&
			mv -v ${glibc_org_src_dir} ${glibc_src_dir_crs_1st}) || return 1
...
EOF
	...
	[ "${enable_check}" != yes ] ||
		make -C ${glibc_bld_dir_crs_1st} -j ${jobs} DESTDIR=${sysroot} -k check || return 1
...
install_cross_functional_gcc()
{
	...
	[ "${enable_check}" != yes ] ||
		LIBS=-lgcc_s make -C ${gcc_bld_dir_crs_3rd} -j ${jobs} -k check || return 1
...
install_cross_gdb()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${gdb_bld_dir_crs} -j ${jobs} -k check || return 1
...
install_mingw_w64_header()
{
	...
	[ -d ${mingw_w64_src_dir_crs_hdr} ] ||
		(unpack ${mingw_w64_org_src_dir} ${mingw_w64_src_base} &&
			mv -v ${mingw_w64_org_src_dir} ${mingw_w64_src_dir_crs_hdr}) || return 1
	...
	[ "${enable_check}" != yes ] ||
		make -C ${mingw_w64_bld_dir_crs_hdr} -j ${jobs} -k check || return 1
...
install_mingw_w64_crt()
{
	[ -d ${mingw_w64_src_dir_crs_1st} ] ||
		(unpack ${mingw_w64_org_src_dir} ${mingw_w64_src_base} &&
			mv -v ${mingw_w64_org_src_dir} ${mingw_w64_src_dir_crs_1st}) || return 1
	...
	[ "${enable_check}" != yes ] ||
		make -C ${mingw_w64_bld_dir_crs_1st} -j ${jobs} -k check || return 1
...
install_native_python()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${Python_org_src_dir} -j ${jobs} -k test || return 1
...
install_native_ruby()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${ruby_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_go()
{
	...
	[ ! -d ${prefix}/go ] || rm -fr ${prefix}/go || return 1
	mv -v ${go_org_src_dir} ${prefix}/go || return 1
...
install_native_tcl()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${tcl_org_src_dir}/unix -j ${jobs} -k test || return 1
...
install_native_tk()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${tk_org_src_dir}/unix -j ${jobs} -k test || return 1
...
install_native_libunistring()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${libunistring_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_libatomic_ops()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${libatomic_ops_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_guile()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${guile_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_yasm()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${yasm_org_src_dir} -j ${jobs} -k check || return 1
...
install_native_x264()
{
	...
	[ -d ${x264_org_src_dir} ] ||
		...
		mv -v ${x264_src_base}/x264-snapshot-* ${x264_org_src_dir}) || return 1
	(cd ${x264_org_src_dir}
...
install_native_x265()
{
	...
	[ -d ${x265_org_src_dir} ] ||
		...
		mv -v ${x265_src_base}/x265_${x265_ver} ${x265_org_src_dir}) || return 1
...
install_native_opencv()
{
	...
	[ "${enable_check}" != yes ] ||
		make -C ${opencv_bld_dir_ntv} -j ${jobs} -k check || return 1
...
install_crossed_native_binutils()
{
	...
	[ -d ${binutils_src_dir_crs_ntv} ] ||
		(unpack ${binutils_org_src_dir} ${binutils_src_base} &&
			mv -v ${binutils_org_src_dir} ${binutils_src_dir_crs_ntv}) || return 1
...
install_crossed_native_gmp()
{
	...
	[ -d ${gmp_src_dir_crs_ntv} ] ||
		(unpack ${gmp_org_src_dir} ${gmp_src_base} &&
			mv -v ${gmp_org_src_dir} ${gmp_src_dir_crs_ntv}) || return 1
...
install_crossed_native_mpfr()
{
	...
	[ -d ${mpfr_src_dir_crs_ntv} ] ||
		(unpack ${mpfr_org_src_dir} ${mpfr_src_base} &&
			mv -v ${mpfr_org_src_dir} ${mpfr_src_dir_crs_ntv}) || return 1
...
install_crossed_native_mpc()
{
	...
	[ -d ${mpc_src_dir_crs_ntv} ] ||
		(unpack ${mpc_org_src_dir} ${mpc_src_base} &&
			mv -v ${mpc_org_src_dir} ${mpc_src_dir_crs_ntv}) || return 1
...
install_crossed_native_zlib()
{
	...
	[ -d ${zlib_src_dir_crs_ntv} ] ||
		(unpack ${zlib_org_src_dir} ${zlib_src_base} &&
			mv -v ${zlib_org_src_dir} ${zlib_src_dir_crs_ntv}) || return 1
...
install_crossed_native_libpng()
{
	...
	[ -d ${libpng_src_dir_crs_ntv} ] ||
		(unpack ${libpng_org_src_dir} ${libpng_src_base} &&
			mv -v ${libpng_org_src_dir} ${libpng_src_dir_crs_ntv}) || return 1
...
install_crossed_native_libtiff()
{
	...
	[ -d ${tiff_src_dir_crs_ntv} ] ||
		(unpack ${tiff_org_src_dir} ${tiff_src_base} &&
			mv -v ${tiff_org_src_dir} ${tiff_src_dir_crs_ntv}) || return 1
